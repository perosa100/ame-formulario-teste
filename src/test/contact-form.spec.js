...
describe('ContactForm Component', () => {
  ...
  beforeEach(() => {
    ...
    // Form Wrapper
    wrapper = mount(
      <ContactForm data={contactData} onChange={onChange} onSubmit={onSubmit} />
    )
  ...
  it('should call onChanges with updated contact data when `options` field changes', () => {
    ...
    fields.options.forEach(option => {
      ...
      expect(
        ...
        `onChange wasn't called when '${key}' changed`
      ).to.be.false
    })
  })
  ...
  // o teste foi alterado pois a funçao nao reporna undefined, e sim retorna false
  ...
  it('should show correct initial value for `options` field', () => {
    ...
    fields.options.forEach(option => {
      if (option.instance().value == contactData.option) {
        ...
        expect(
          option,
          `Incorrect 'options' field is checked`
        ).not.to.be.checked()
      }
    })
  })
  ...
  it('should update `options` field', () => {
    ...
    fields.options.forEach(option => {
      if (option.instance().value == changedValue) {
        expect(
          option,
          `Incorrect 'options' field is checked after update`
        ).to.be.checked()
      } else {
        expect(
          option,
  ...
  function testField(name, field, key, changedValue) {
    // Test initial value
    expect(field, `Field '${name}' initial value is missing`).to.have.value(
      contactData[key]
    )
    // Change Value
    wrapper.setProps({
    ...
    expect(
      field,
      `Field '${name}' value is not updated correctly`
    ).to.have.value(changedValue)
  }

  it('should render options with labels using component`s `options` values', () => {
    let optionsConfigured = wrapper
      .instance()
      .options.map(option => option.label)
    let optionsRendered = fields.select
      .find('option')
      .map(node => node.instance().textContent)
    expect(
      optionsConfigured,
      'Select question type options are not rendered correctly'
    ).to.deep.eq(optionsRendered)
  })
})
